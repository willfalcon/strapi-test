...
module.exports = {
  ...
  fetch: (params) => {
    ...
    return Clients
      .findOne(_.pick(params, _.keys(Clients.schema.paths)))
      .populate(populate);
  },
  ...
  add: async (values) => {
    ...
    const entry = await Clients.create(data);

    // Create relational data and return the entry.
    return Clients.updateRelations({ _id: entry.id, values: relations });
  },
  ...
  edit: async (params, values) => {
    // Extract values related to relational data.
    const relations = _.pick(values, Clients.associations.map(a => a.alias));
    const data = _.omit(values, Clients.associations.map(a => a.alias));

    // Update entry with no-relational data.
    const entry = await Clients.update(params, data, { multi: true });
  ...
  remove: async params => {
    ...
    await Promise.all(
      Clients.associations.map(async association => {
  ...
  search: async (params) => {
    ...
    const $or = Object.keys(Clients.attributes).reduce((acc, curr) => {
      switch (Clients.attributes[curr].type) {
        case 'integer':
        case 'float':
        case 'decimal':
